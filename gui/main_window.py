...
class App:
    ...
    def start_process(self):
        ...
        self.engine = YoloTester(
            ...
            use_ocr=True  # Truyền giá trị bật/tắt OCR
        )
    ...
    def on_canvas_click(self, event):
        ...
        for tid, obj in self.engine.current_objects.items():
            ...
            if x1 <= x <= x2 and y1 <= y <= y2:
                self.engine.request_ocr(tid)   # 👈 OCR khi click
    ...
    def show_ship_detail(self, track_id, obj):
        ...
        # Ưu tiên show vùng chữ detect được
        if obj.get("text_crop") is not None:
            img = obj["text_crop"]

        # nếu chưa detect chữ thì show tàu
        elif obj["crop"] is not None:
            img = obj["crop"]
